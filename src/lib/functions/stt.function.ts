...
async function fetchPluelySTT(audio: File | Blob): Promise<string> {
  try {
    ...
    if (response.success && response.transcription) {
      ...
      return response.error || "转录失败";
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return `Vivox STT Error: ${errorMessage}`;
  }
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    if (!provider) throw new Error("未提供提供商");
    if (!selectedProvider) throw new Error("未提供所选提供商");
    if (!audio) throw new Error("需要音频文件");
    ...
    // [二开] 修复: 外部HTTP请求使用tauriFetch绕过CORS
    ...
    try {
      ...
      throw new Error(`网络错误: ${e instanceof Error ? e.message : e}`);
    }
